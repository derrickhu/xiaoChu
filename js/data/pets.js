...
const PETS = {
  ...
  metal: [
    ...
    { id:'m6',  name:'金光剑灵',  atk:11, skill:{ name:'剑气纵横', desc:'Combo+3，且本次Combo不会断', type:'comboPlusNeverBreak', count:3 }, cd:5 },
...
// 从本局宠物池中随机获取一只宠物
// ownedIds: 可选，已拥有宠物ID集合，50%概率优先抽已拥有的（促进升星）
function randomPetFromPool(sessionPool, ownedIds) {
  if (!sessionPool || sessionPool.length === 0) return randomPet()
  // 50%概率偏向已有宠物（促进升星体验）
  if (ownedIds && ownedIds.size > 0 && Math.random() < 0.5) {
    const owned = sessionPool.filter(p => ownedIds.has(p.id))
    if (owned.length > 0) {
...
// 生成本局宠物池：每属性随机3只，共15只（提高同ID命中率，加速升星）
function generateSessionPetPool() {
  ...
  for (const attr of ['metal','wood','water','fire','earth']) {
    ...
    // 取前3只
    for (let k = 0; k < 3 && k < attrPets.length; k++) {
