...
async function syncOnce(storage, onInviterReward) {
  const serverId = (storage && storage.serverId) || (cloudSync.getServerId && cloudSync.getServerId()) || 's1'
  if (_syncedServerId === serverId) return
  if (!P.isWeChat) return  // 抖音暂无对应邀请活动
  if (!cloudSync.isReady()) return
  const openid = cloudSync.getOpenid && cloudSync.getOpenid()
  if (!openid) return
  _syncedServerId = serverId
...
// 手动重置（通常用于测试）
function _reset() { _syncedServerId = '' }
