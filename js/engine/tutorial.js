...
function _getCurRound() {
  if (_stageMode) {
    return _stageOverrideData || null
  }
...
function onElim(g) {
  ...
  _guideDone = true
  // 秘境简化教学：消除完成后立即结束教学，回归自由操作
  if (_stageMode) {
    finishStageTutorial(g)
  }
}
...
function getGuideData() {
  ...
  // 秘境简化教学：直接使用覆盖数据
  if (_stageMode && _stageOverrideData) {
    const rd = _stageOverrideData
    return {
      step: 0, round: 0, totalRounds: 1,
      title: rd.title || '转珠攻击',
      guide: rd.guide || null,
      msgs: rd.msg || [],
      phase: _phase,
